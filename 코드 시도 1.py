...
def calculate_stress_tensor(bending_stress, shear_stress):
    """
    응력 텐서를 구성하여 가장 큰 값의 유효 숫자에 따라 출력합니다.
    ...
    Returns:
    - stress_tensor: 3x3 응력 텐서 (리스트로 표현)
    """
    # 응력 텐서 구성
    stress_tensor = np.array([
        [bending_stress, shear_stress, 0],  # [sigma_xx, sigma_xy, sigma_xz]
        [shear_stress, 0, 0],               # [sigma_yx, sigma_yy, sigma_yz]
        [0, 0, 0]                           # [sigma_zx, sigma_zy, sigma_zz]
    ])
    
    # 응력 텐서의 절대값 중 가장 큰 값 찾기
    max_value = np.max(np.abs(stress_tensor))
    max_digits = len(f"{int(max_value)}")  # 가장 큰 값의 정수 자릿수
    precision = 4 - max_digits  # 총 4자리로 출력

    # 텐서 형식 지정 (최대 유효숫자 4자리로 맞추기)
    formatted_tensor = np.round(stress_tensor, precision)

    # 응력 텐서 출력
    print("\n응력 텐서:")
    for row in formatted_tensor:
        print(" | ".join(f"{val:>{max_digits + precision + 2}.{precision}f}" for val in row))
...
calculate_stress_tensor(sigma_xx, sigma_xy)