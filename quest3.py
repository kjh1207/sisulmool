...
while True:
   ...
   # 926 ~ 936 줄의 기존 처리 내용
    if matching_crack:
        ...
        # 균열 영역 확인
        if -bridge_data["단면 높이"] / 2 <= y <= -bridge_data["단면 높이"] / 2 + crack_depth:
...
def calculate_principal_stresses_2d(stress_tensor):
    ...
    return sigma_1 * 0.001, sigma_2 * 0.001
    ...
    while True:
        ...
        if matching_crack:
            ...
            if -bridge_data["단면 높이"] / 2 <= y <= -bridge_data["단면 높이"] / 2 + crack_depth:
                ...
                sigma_xy = 0
                print("경고: 해당 위치는 균열 영역에 포함됩니다. 응력은 0으로 간주됩니다.")
            elif -bridge_data["단면 높이"] / 2 + crack_depth < y <= -bridge_data["단면 높이"] / 2:
                # 균열 윗부분
        ...
        print("\n--- 응력 결과 ---")
        print(f"위치 x: {x}, y: {y}, z: {z}")
        print(f"휨응력 (σ_xx): {sigma_xx:.3f} MPa")
        print(f"전단응력 (τ_xy): {sigma_xy:.3f} MPa")
        print(f"주응력:")
        ...
        # 사용자 입력에 따라 종료 여부 결정
        repeat = input("\n다른 위치를 입력하려면 'y', 프로그램을 종료하시려면 'n'을 입력하세요: ").strip().lower()
        if repeat == 'y':
            continue
        elif repeat == 'n':
            print("프로그램을 종료합니다.")
...
def plot_mohr_failure(sigma_ult_c, sigma_ult_t, tau_ult, sigma_1, sigma_2):
    """
    Plot the Mohr failure envelope and principal stresses.
